...
if(el('pg-d').classList.contains('on')&&userName)debouncedRenderDashboard();
  ...
  db.ref('bills').on('value',function(snap){bills=[];snap.forEach(function(c){bills.push(c.val());});renderBills();if(el('pg-d').classList.contains('on')&&userName)debouncedRenderDashboard();});
  attachDinnerQListener();
  db.ref('planner/'+dKey(getWeekDates(0)[0])).on('value',function(){if(el('pg-d').classList.contains('on')&&userName)debouncedRenderDashboard();});
